...
ROOT_DIR = os.path.realpath(os.path.join(TOOLS_DIR, '..', 'g_stock'))
...
def update_(locale=None):
    ...
    cmd = 'xgettext --no-wrap --sort-by-file --default-domain=g_stock ' \
          '--copyright-holder="yɛlɛman s.à.r.l" --package-name="g_stock" ' \
          '--package-version="%s" ' \
          '--msgid-bugs-address=bugs@yeleman.com ' \
          '--language=Python --keyword=_ --add-comments ' \
          '--output=g_stock.pot ' % version + ' '.join(dirs_str)
    return exec_cmd(cmd)


def merge_(locale):
    return exec_cmd('msgmerge --update --no-wrap --sort-by-file ' \
                    '%s/LC_MESSAGES/g_stock.po g_stock.pot' % locale)


def compile_(locale):
    return exec_cmd('msgfmt --output-file=%s/LC_MESSAGES/g_stock.mo ' \
                    '%s/LC_MESSAGES/g_stock.po' % (locale, locale))


def new_(locale):
    return exec_cmd('msginit --locale=%s --no-wrap ', \
                    '--output-file=%s/LC_MESSAGES/g_stock.po ' \
                    '--input=g_stock.pot' % (locale, locale))
