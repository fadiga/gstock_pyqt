...
def get_time_pagination(year, duration, duration_number):
    """
    navigation entre les dates annee, mois, week """
    ...
    if duration == "week":
        ...
        # la date de la semaine apres celle qu'on affiche
        ...
        # Verification que la semaine d'aujourd'hui est a afficher ou non
        ...
        # l'adresse pour afficher l'annee d'aujourd'hui
        ...
        # la date du mois apres celui qu'on affiche
        ...
        current_date = date(year, 1, 1)
         ...
         # la date de l'annee avant celle qu'on affiche
        previous_date = date(current_date.year - 1,
                             current_date.month,
                             current_date.day)

        # la date de l'annee apres celle qu'on affiche
        next_date = date(current_date.year + 1,
                             current_date.month,
                             current_date.day)

        # Verification que l'annee d'aujourd'hui est a afficher ou non
        ...
        # l'adresse pour afficher l'annee precedent
        previous_date_url = reverse(url,
                                    args=additional_args + \
                                            (previous_date.year,))

        # l'adresse pour afficher l'annee suivant
        next_date_url = reverse(url,
                                args=additional_args + (next_date.year,))

        # formatage de l'affichage des annees
        current_date_ = current_date.strftime("%Y")
        previous_date_ = previous_date.strftime("%Y")
        next_date_ = next_date.strftime("%Y")
        todays_date = u"Cette annee"
...
# TODO : faire de ce bebe mamouth un middleware ou un context processor
def get_duration_pagination(year, duration,
    ...
    if duration == "week":
        ...
        # l'adresse pour afficher l'annee
